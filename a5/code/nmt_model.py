...
class NMT(nn.Module):
    ...
    def forward(
        ...
        log-likelihood of target sentences under the language models learned by
        the NMT system.
        ...
        @returns scores (Tensor): a variable/tensor of shape (b, ) representing
                                  the log-likelihood of generating the
                                  gold-standard target sentence for each example
                                  in the input batch. Here b = batch size.
        """
        ...
        # TODO:
        # Modify the code lines above as needed to fetch the character-level
        # tensor to feed into encode() and decode(). You should:
        # - Keep `target_padded` from A4 code above for predictions
        # - Add `source_padded_chars` for character level padded encodings for
        #   source.
        # - Add `target_padded_chars` for character level padded encodings for
        #   target.
        # - Modify calls to encode() and decode() to use the character level
        #   encodings.
    ...
    def encode(
        ...
        states. Additionally, take the final states of the encoder and project
        them to obtain initial states for decoder.
        @param source_padded (Tensor): Tensor of padded source sentences with
                                       shape (src_len, b, max_word_length),
                                       where b = batch_size,
                                       src_len = maximum source sentence length.
                                       Note that these have already been sorted
                                       in order of longest to shortest sentence.
        @param source_lengths (List[int]): List of actual lengths for each of
                                       the source sentences in the batch

        @returns enc_hiddens (Tensor): Tensor of hidden units with shape
                                       (b, src_len, h*2), where
                                       b = batch size,
                                       src_len = maximum source sentence length
                                       h = hidden size.
        @returns dec_init_state (tuple(Tensor, Tensor)):
                                       Tuple of tensors representing the
                                       decoder's initial hidden state and cell.
        """
        enc_hiddens, dec_init_state = None, None

        # COPY OVER YOUR CODE FROM ASSIGNMENT 4
        # Except replace "self.model_embeddings.source" with
        # "self.model_embeddings_source"

        # 1. Embed the padded source sentences.
        # Tensor: (src_len, b, embed_word_size)
        ...
        # - After encoder, apply pad packing to enc hidden states and reshape.
    ...
    def generate_sent_masks(self, enc_hiddens: torch.Tensor, source_lengths: List[int]) -> torch.Tensor:
        ...
        @param enc_hiddens (Tensor): encodings of shape (b, src_len, 2*h), where
                                     b = batch size, src_len = max source length,
                                     h = hidden size.
        @param source_lengths (List[int]): List of actual lengths for each of
                                     the sentences in the batch.
        @returns enc_masks (Tensor): Tensor of sentence masks of shape
                                     (b, src_len), where
